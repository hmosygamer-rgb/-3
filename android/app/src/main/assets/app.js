...
window.addEventListener('DOMContentLoaded',()=>{ if(window.AndroidApp) window.AndroidApp.setDark(initialTheme==='dark'); });
...
$('#routeBtn').addEventListener('click',()=>{const m=members[selected];const url=`https://www.google.com/maps/dir/?api=1&destination=${m.coords.join(',')}`;if(window.AndroidApp)window.AndroidApp.openMap(url);else window.open(url,'_blank','noopener');});
...
$('#themeBtn').addEventListener('click',()=>{
  ...
  if(window.AndroidApp) window.AndroidApp.setDark(dark);
